...
Public Class RWP_DashBorad_old
    ...
    Public Sub allotment_cast(ByVal fnyear As Integer)
        ...
        Dim str As String = " select sum(CASE WHEN YEARS='" & fnyear & "' and qtr between 1 and 3 or years='" & (fnyear + 1) & "' and qtr=4 THEN wheel_qty ELSE 0 END) sumofvalue from WTA_ALLOTMENT$"
        Dim str1 As String = "select sum(CASE WHEN YEARS='" & fnyear & "' and qtr between 1 and 3 or years='" & (fnyear + 1) & "' and qtr=4 THEN Qty_M ELSE 0 END) sumofvalue1 from wheel_dispatch_casting$ where wheel_category=3"
...
ELSE 0 END) sum3  from WHEEL_DISPATCH_CASTING$ p where  qtr='" & category & "' group by fyear"

...
Public Class MagProcessingNavigator
    ...
    Protected Sub btnstock_Click(sender As Object, e As EventArgs) Handles btnstock.Click
        ...
        Try
            ...
            If n > 0 Then
                UDone = f2()
                If UDone Then
                    lblmsg.Text = "Magna Records Updation Successful!"
                    clearscreen()
                End If
            Else
                SDone = f1()
        ...
        sqlstr1 = "select ROW_NUMBER() OVER(ORDER BY Magid asc) AS Magid,CONVERT(varchar, TestDate,106) TestDate,Shift ,ProdType,CopeInsp , DragInsp, LineNumber,HeatNo, WheelNo ,WheelType,DefectCodes, MPI_Remarks,  Machining, Grinding, BHN ,MbsCurrent, BCF, UtBatch , UtWheelNo , UtStatus , UtDefectCode,  FinalStatus from mm_magnaglow_new_shiftwiserecords where TestDate='" & Format(CDate(txtdate.Text), "MM/dd/yyyy") & "' and Shift='" & rblshift.SelectedItem.Value & "' order by Magid desc"
    ...
    Public Function f1()
        ...
        If RBLProductType.SelectedItem.Value = "BGC" And Val(txtbhnone.Text) < 322 And Val(txtbhnone.Text) > 254 Then
            BhnRange = "VALID"
        ElseIf RBLProductType.SelectedItem.Value = "BGC" And Val(txtbhnone.Text) > 321 Then
            BhnRange = "INVALID"
        ElseIf RBLProductType.SelectedItem.Value = "BGC" And Val(txtbhnone.Text) < 255 And Val(txtbhnone.Text) > 0 Then
            BhnRange = "INVALID"
        End If
        If RBLProductType.SelectedItem.Value = "BOXN" And Val(txtbhnone.Text) < 342 And Val(txtbhnone.Text) > 276 Then
            BhnRange = "VALID"
        ElseIf RBLProductType.SelectedItem.Value = "BOXN" And Val(txtbhnone.Text) > 341 Then
            BhnRange = "INVALID"
        ElseIf RBLProductType.SelectedItem.Value = "BOXN" And Val(txtbhnone.Text) < 277 And Val(txtbhnone.Text) > 0 Then
            BhnRange = "INVALID"
        End If
        ...
        If oprtype = "OK" And ddutstatus.SelectedItem.Value = "Passed" And BhnRange = "VALID" Then
            status = "STOCK"
        ElseIf oprtype = "OK" And (ddutstatus.SelectedItem.Value = "Passed" Or ddutstatus.SelectedItem.Value = "UnChecked" Or ddutstatus.SelectedItem.Value = "Reject") And BhnRange = "INVALID" Then
            status = "RHT"
        ...
        ElseIf oprtype = "RLM" And ddOperReqd.SelectedItem.Value = "NODEFECT" And BhnRange = "VALID" Then
            status = "RLM"
            TxtMPIRemarks.Text = ""
        ElseIf ddOperReqd.SelectedItem.Value = "NODEFECT" And (oprtype = "RLB" Or oprtype = "RLM" Or oprtype = "RLC") And BhnRange = "INVALID" Then
            status = "RHT"
            TxtMPIRemarks.Text = ""
        ElseIf Val(txtbhnone.Text) = 0 Then
            status = "RLB"
            TxtMPIRemarks.Text = ""
        End If
	...
	OperDoneWFPS,HTBatch ,HTStatus,TxnDateTime) values  ('" & Format(CDate(txtdate.Text), "MM/dd/yyyy") & "','" & rblshift.SelectedItem.Value & "',
                    ...
                    '" & status & "' ,'" & TxtWFPSdone.Text & "','" & txthtbatch.Text & "','" & ddhtstatus.SelectedItem.Value & "',getdate() )"
    ...
    Public Function f2()
        ...
        Try
            ...
            cmd.Connection.Open()
            cmd.CommandText = ("Update mm_magnaglow_new_shiftwiserecords set BCF=@BCF where TestDate=@mdate and Shift=@shift and  WheelNo=@WLNo and HeatNo=@HTNo and FinalStatus=@FStatus")
    ...
    Private Sub validatewheel()
        ...
        Catch exp As Exception
            ...
            lblmsg.Text = " Please enter valid Wheel Number"
        End Try
        SetFocus(txtbhnone)
    End Sub
    ...
    Private Sub GetLine3newdata()
        ...
        dt = Nothing
    End Sub
    Private Sub setL3Grinding()
        If ddOperReqd.SelectedItem.Value = "GRINDING" Or ddOperReqd.SelectedItem.Value = "MCNGRD" Then
            TxtGrinding.Text = "GRE"
        Else
            TxtGrinding.Text = ""
        End If
    End Sub
    Private Sub setL1Grinding()
        If ddOperReqd.SelectedItem.Value = "GRINDING" Or ddOperReqd.SelectedItem.Value = "MCNGRD" Then
            TxtGrinding.Text = "GR"
        Else
            TxtGrinding.Text = ""
    ...
    Protected Sub MAGNAGRID_SelectedIndexChanged(sender As Object, e As EventArgs) Handles MAGNAGRID.SelectedIndexChanged
        ...
        txtdate.Text = MAGNAGRID.Items(i).Cells(2).Text
        rblshift.SelectedItem.Value = Trim(MAGNAGRID.Items(i).Cells(3).Text)
        RBLProductType.SelectedValue = Trim(MAGNAGRID.Items(i).Cells(4).Text)
        txtheat.Text = Trim(MAGNAGRID.Items(i).Cells(5).Text)
        txtwheel.Text = MAGNAGRID.Items(i).Cells(6).Text
        txtFinalstatus.Text = MAGNAGRID.Items(i).Cells(7).Text
        ddline.SelectedItem.Value = Trim(MAGNAGRID.Items(i).Cells(8).Text)
        TxtDefectCode.Text = MAGNAGRID.Items(i).Cells(9).Text
        TxtMPIRemarks.Text = MAGNAGRID.Items(i).Cells(10).Text
        txtbhnone.Text = MAGNAGRID.Items(i).Cells(11).Text
        txtutwheel.Text = MAGNAGRID.Items(i).Cells(12).Text
        ddutstatus.SelectedItem.Value = Trim(MAGNAGRID.Items(i).Cells(13).Text)
        TxtUTDefectCode.Text = MAGNAGRID.Items(i).Cells(14).Text
        txtcope.Text = Trim(MAGNAGRID.Items(i).Cells(15).Text)
        txtdrag.Text = Trim(MAGNAGRID.Items(i).Cells(16).Text)
        ddwhltype.SelectedItem.Value = Trim(MAGNAGRID.Items(i).Cells(17).Text)
        TxtMachining.Text = MAGNAGRID.Items(i).Cells(18).Text
        TxtGrinding.Text = MAGNAGRID.Items(i).Cells(19).Text
        txtamm.Text = MAGNAGRID.Items(i).Cells(20).Text
        txtbcf.Text = MAGNAGRID.Items(i).Cells(21).Text
        txtutbatch.Text = MAGNAGRID.Items(i).Cells(22).Text

        btnstock.Text = "UPDATE RECORDS"
        txtFinalstatus.Enabled = True
    End Sub
    ...
    Private Sub GetProductType()
        ...
        Dim heatno As String = txtheat.Text.Trim
        cmd.Connection.Open()
        cmd.CommandText = ("select wheel_type from mm_pouring where heat_number = @heatno and pouring_order=1")
        cmd.Parameters.AddWithValue("@heatno", heatno)
        Try
            Dim WHLtype As String = cmd.ExecuteScalar()
            Response.Write(WHLtype)
            If WHLtype = "BOXN WHL                                          " Then
                RBLProductType.SelectedValue = "BOXN"
            ElseIf WHLtype = "ICF WHL                                           " Then
                RBLProductType.SelectedValue = "BGC"
            ElseIf WHLtype Like "BLC WHL" Then
                RBLProductType.SelectedValue = "BLC"
            End If
    ...
    Protected Sub btnMagStatus_Click(sender As Object, e As EventArgs) Handles btnMagStatus.Click
        Dim url As String
        url = "http://rwpbela3.southindia.cloudapp.azure.com:8080/BOE/CMC/1903072256/CrystalReports/viewrpt.cwr?id=46473&apsuser=Administrator&apspassword=Cris@123&apsauthtype=secEnterprise&user0=crissqlserver&password0=Cris@BelaQwertPoiuy&promptonrefresh=0" &
